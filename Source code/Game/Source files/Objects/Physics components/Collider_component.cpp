...
namespace Game
{
	namespace Objects
	{
		Collider_component::Collider_component(Game_object* container, std::unique_ptr<Geometry::Shape<double>> shape, std::vector<std::string> layers, std::vector<std::string> check_layers, Physics::Collision_system& collision_system) : Component{ container }, shape_{ move(shape) }, layers_{ layers }, check_layers_{check_layers}, collision_system_ { collision_system } // TODO: Look up forwarding
		{
		}
		...
		{
			auto component = new Collider_component{game_object, std::unique_ptr<Geometry::Shape<double>>{Geometry::Shape<double>::from_json(j.at("shape"))}, j.at("layers").get<std::vector<std::string>>(), j.at("check_layers").get<std::vector<std::string>>(), loader.physics_loader().collision_system_};
		...
		{
			auto collisions = collision_system_.get_collisions(*this);

			auto old_pos = game_object_->get_position();
			...
			for (auto& col_pair : collisions)
			{
				if (col_pair.second == "solid")
				{
					...
					if (abs((new_pos - message.start_position).get_magnitude()) < abs((game_object_->get_position() - message.start_position).get_magnitude()))
					{
