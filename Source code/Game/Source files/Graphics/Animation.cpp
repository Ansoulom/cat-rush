...
namespace Game
{
	namespace Graphics
	{
		Animation::Animation(std::string texture, int frames, int rows, double frame_rate) : texture_name{texture}, frames{frames}, rows{rows}, current_frame{0}, current_row{0}, frame_rate{frame_rate}, timer{Timer::Seconds{1.0} / frame_rate}
		{
			if(frames < 1)
			{
		...
		Animation::Frame Animation::get_current_frame(const Texture_manager& tm) const
		{
			...
			auto width = texture->get_width() / frames;
			auto height = texture->get_height() / rows;
			auto clip = Geometry::Rectangle<int>{{current_frame * width, current_row * height}, width, height, Geometry::Pivot_point::top_left};
		...
		{
			if(timer.update(time_passed))
			{
				timer.reset_to_surplus();
		...
		{
			return {{"texture", texture_name},{"frames", frames},{"rows", rows},{"frame_rate", frame_rate}};
		}


		Animation::Frame::Frame(Texture& texture, const Geometry::Rectangle<int>& clip) : texture{texture}, clip{clip}
		{
